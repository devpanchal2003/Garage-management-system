...
def service(request):
    ...
    if request.method == 'POST' or user.is_authenticated:
        ...
        if not name:
            ...
            messages.error(request, "Address must be at least 15 characters")
        elif not pin_code:
            messages.error(request, "Pin code is required")
        elif len(pin_code) < 6:
            messages.error(request, "Pin code must be 6 characters")
        elif not phone:
            messages.error(request, "Phone is required")
        elif len(phone) < 9:
            messages.error(request, "Phone must be at least 9 characters")
        elif not email:
            messages.error(request, "Email is required")
        elif len(email) < 9:
            messages.error(request, "Email must be at least 9 characters")
        elif not busno:
            messages.error(request, "Bus number is required")
        elif len(busno) < 7:
            messages.error(request, "Bus number must be at least 7 characters")
        elif not issue:
            messages.error(request, "Issue is required")
        elif len(issue) < 20:
            messages.error(request, "Issue must be at least 20 characters")
        else:
            # Save the appointment
            appointement = Appointement(
                name=name, email=email, phone=phone, busno=busno,
                issue=issue, address=address, pin_code=pin_code, user=user
            )
            appointement.save()

            # Get the appointment ID and calculate the time
            ...
            subject = "Appointment Confirmation - Daddy's Garage"
            from_email = settings.DEFAULT_FROM_EMAIL  # Using the email from settings
            to = email

            # Render the email template with dynamic values
            html_content = render_to_string('mail_template.html', {
                'id': id, 'user': user, 'name': name, 'day': day, 'mon': month, 'yr': year
            })
            ...
            messages.success(request, "We will contact you within 48 hours.")
